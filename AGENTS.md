...
`loader.toml` (the single declarative manifest the build tool derives EVERYTHING
from: targets, features, components, FHS package set, manifest identity +
classification, drive layout, and the Makefile target list) · `usb.lock` (pins) ·
`vendor.lock.json` (FOD hashes) · `flake.nix` (devshell + `packages.vendor`/
`ollamaComponents` FODs/repack + `.#spa` + `launcher-<target>` + `.#xtask` built from
the loader submodule) · `Makefile` (GENERATED by `xtask gen-makefile` from
`loader.toml [make]`, but committed so it can trigger xtask) · `scripts/` (the
impure/project steps: download, build-openwebui, make-runtime, seed-models, tests) ·
...
subcommands generic), `nix/loader` (the generic packers `mkSqfs`/`mkDmg`/
`mkClosureSqfs` + the `buildFHSEnv` builder + the `dist/.stores` reader), and the
generic packing scripts (`store-import`, `nix-component`, `import-build-component`,
`pack-component`, `lib`, `bundle`, `make-usb-image`). A second product vendors this
submodule, writes its own `loader.toml`, and gets the same build pipeline. The
scripts honour `PLANAI_REPO_ROOT` (exported by `xtask build` before ninja) so a
vendored `lib.sh` keeps `REPO_ROOT` = the project root. `self.submodules = true`
brings it into the flake source (same as `plan-ai-design`).
