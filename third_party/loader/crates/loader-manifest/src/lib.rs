//! Project-agnostic update-manifest schema + logic, used by BOTH the build tool
//! (the loader xtask, which generates manifests for the USB image + the update
//! server) and the loader-core updater (which diffs a local manifest against the
//! remote one and applies the delta). One source of truth ⇒ the generator and the
//! updater can never drift.
//!
//! Platform/feature *classification* is **data-driven** via [`ClassifyTable`]: the
//! build tool builds one from the project's `loader.toml` and threads it into
//! [`generate`]. The launcher updater never classifies at runtime — it diffs
//! already-classified [`Entry`] data — so the table is a build-side concern only.
...
/// The auto-updater's runtime status DTOs (the generic updater↔UI contract).
pub mod update;
pub use update::{UpdateState, UpdateStatus};

/// A reasonable default update server when a manifest carries none. Projects set
/// their own via `[manifest].update_url` in `loader.toml` → [`generate`]'s arg.
pub const DEFAULT_UPDATE_URL: &str = "https://usb-update.plan.ai";

/// Data-driven platform + feature classification — the project's `loader.toml`
/// `[manifest.classify]` / `[manifest.feature_classify]` + `[targets].known`,
/// reified so the generator carries no hardcoded plan.ai tables.
///
/// - `exact`: full drive-relative path → the single target it belongs to.
/// - `group_targets`: the `components/<target>/…` group segments treated as
///   authoritative (everything under a known target group is that target).
/// - `substring`: OS substring → the target arches it expands to (a bare OS can't
///   name its arch, so e.g. `linux` ⇒ both linux arches).
/// - `tools_targets`: targets a `tools/` path serves (vestigial; static-musl tools).
/// - `feature`: optional-feature name → the component basename prefixes that own it.
#[derive(Debug, Clone, Default)]
pub struct ClassifyTable {
    pub exact: Vec<(String, String)>,
    pub group_targets: Vec<String>,
    pub substring: Vec<(String, Vec<String>)>,
    pub tools_targets: Vec<String>,
    pub feature: Vec<(String, Vec<String>)>,
}

impl ClassifyTable {
    /// The historical plan.ai table — kept so the in-crate tests (and any consumer
    /// that wants the original behaviour) need no `loader.toml`. New products build
    /// their own table from their manifest instead of using this.
    pub fn plan_ai_default() -> Self {
        let s = |a: &str| a.to_string();
        ClassifyTable {
            exact: vec![
                (s("plan-ai.linux-x64.exe"), s("linux-x64")),
                (s("plan-ai.linux-arm64.exe"), s("linux-arm64")),
                (s("plan-ai.exe"), s("win-x64")),
                (s("plan-ai.dmg"), s("mac-arm64")),
            ],
            group_targets: KNOWN_TARGET_KEYS.iter().map(|t| s(t)).collect(),
            substring: vec![
                (s("linux"), vec![s("linux-x64"), s("linux-arm64")]),
                (s("nixos"), vec![s("linux-x64"), s("linux-arm64")]),
                (s("darwin"), vec![s("mac-arm64")]),
                (s("-mac-"), vec![s("mac-arm64")]),
                (s("windows"), vec![s("win-x64")]),
                (s("-win-"), vec![s("win-x64")]),
            ],
            tools_targets: vec![s("linux-x64"), s("linux-arm64")],
            feature: vec![
                (s("hermes"), vec![s("hermes")]),
                (s("llamacpp"), vec![s("llamacpp")]),
                (s("openwebui"), vec![s("runtime-"), s("ow-assets")]),
            ],
        }
    }

    /// Which platform(s) need a drive-relative path. Lowercase, group-authoritative,
    /// then exact, then substring; anything unmatched is shared ("all").
    pub fn classify(&self, rel: &str) -> Vec<String> {
        let p = rel.to_ascii_lowercase();
        if let Some(rest) = p.strip_prefix("components/") {
            if let Some((seg, _)) = rest.split_once('/') {
                if self.group_targets.iter().any(|t| t == seg) {
                    return vec![seg.to_string()];
                }
            }
        }
        for (name, target) in &self.exact {
            if p == name.to_ascii_lowercase() {
                return vec![target.clone()];
            }
        }
        if p.starts_with("tools/") && !self.tools_targets.is_empty() {
            return self.tools_targets.clone();
        }
        let mut v: Vec<String> = Vec::new();
        for (needle, targets) in &self.substring {
            if p.contains(&needle.to_ascii_lowercase()) {
                for t in targets {
                    if !v.contains(t) {
                        v.push(t.clone());
                    }
    ...
    /// Which optional feature owns a drive-relative path (None = core). Matched on
    /// the component basename so it works for grouped + flat layouts and every
    /// packaging (.squashfs/.dmg/.zip/dir).
    pub fn classify_feature(&self, rel: &str) -> Option<String> {
        let p = rel.to_ascii_lowercase();
        let base = p.rsplit('/').next().unwrap_or(p.as_str());
        for (feature, prefixes) in &self.feature {
            if prefixes.iter().any(|pre| base.starts_with(&pre.to_ascii_lowercase())) {
...
/// `product` + `update_url` come from the project's `loader.toml [manifest]`;
/// `table` (its `[manifest.classify]`/`[manifest.feature_classify]`) drives
/// per-entry platform + feature tagging.
pub fn generate(root: &Path, product: &str, version: &str, commit: &str, update_url: &str, built_at: &str, table: &ClassifyTable) -> io::Result<Manifest> {
    let mut files = Vec::new();
    walk(root, root, &mut files, table)?;
...
fn walk(root: &Path, dir: &Path, out: &mut Vec<Entry>, table: &ClassifyTable) -> io::Result<()> {
...
mod tests {
    ...
    // The tests exercise the historical plan.ai classification; thin shims over the
    // default table keep the assertions readable (and prove the table reproduces it).
    fn classify(rel: &str) -> Vec<String> {
        ClassifyTable::plan_ai_default().classify(rel)
    }
    fn classify_feature(rel: &str) -> Option<String> {
        ClassifyTable::plan_ai_default().classify_feature(rel)
    }
